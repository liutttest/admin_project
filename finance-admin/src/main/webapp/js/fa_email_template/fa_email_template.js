		    ...
		    validator : function(){
		        var varlidator = $('#admin-form').validate({
				    ...
				    messages: {
				    	'tempName' : {
							required : "请输入模板标题",
							maxlength : "模板标题请不要超过20个字符"
						},
						'title' : {
							required : "请输入邮件标题",
							maxlength : "邮件标题请不要超过200个字符"
						},
						'content' : {
							required : "请输入邮件模板内容"
						}
	...
	var build = {
		bind : function() {
			...
			$("tbody").delegate(".setDefault", "click", function() {
				...
				$.ajax({
					url : contentPath + '/faEmailTemplate/setDefault?tid=' + tid,
					type : 'POST',
					dataType : 'json',
					success : function(data) {
						if (data.status == 'success') {
							location.href = contentPath + "/faEmailTemplate";
						}
					},
					error : function() {
					}
			...
			$("tbody").delegate(".delete", "click", function() {
				...
				var tid = $(this).attr("data-id");
				$.ajax({
					url : contentPath + '/faEmailTemplate/remove?tid=' + tid,
					type : 'POST',
					dataType : 'json',
					success : function(data) {
						if (data.status == 'success') {
			...
			$("tbody").delegate(".edit", "click", function() {
				...
				$.ajax({
					url : contentPath + '/faEmailTemplate/update',
					type : 'POST',
					data : data,
					dataType : 'json',
					success : function(data) {
						...
						$('#title').val(faTemplate.title);
//						$('#content').val(faTemplate.content);
						$editor.html(faTemplate.content);
						$('#tid').val(faTemplate.tid);
						$('#edit-modal').text('修改邮件模板');
...
//				$('#content').val('');
			...
			$('#save').on('click', function() {
				...
				param.tid = $('#tid').val();
				$.ajax({
					url : contentPath + '/faEmailTemplate/create',
					type : 'POST',
					data : param,
					dataType : 'json',
					success : function(data) {
